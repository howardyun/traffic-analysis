...
saveDir_pcap = 'collectData/RawDataWithoutAnyProtection_Https/pcap/'
saveDir_json = 'collectData/RawDataWithoutAnyProtection_Https/json/'
saveDir_csv = 'collectData/RawDataWithoutAnyProtection_Https/csv/'
...
def catch(dicewebsite):
    ...
    # 转换捕获数据为 JSON
    convert_pcap_to_json(os.path.join(saveDir_pcap, f'{dicewebsite}_more.pcap'),
                         os.path.join(saveDir_json, f'{dicewebsite}_more.json'))
...
def main():
    ...
    for index, row in dicwebsite_df.iterrows():
        ...
        # 记录到 CSV 文件
        csv_file_path = os.path.join(saveDir_csv, f'{dicewebsite}_packets.csv')
        with open(csv_file_path, 'w') as csv_file:
            csv_file.write('timestamp,size\n')
            for packet in packets:
                direction = "+" if packet['src_ip'] == local_ip else "-"
                csv_file.write(f"{packet['timestamp']},{direction}{packet['size']}\n")
